...
class Batch(object):
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            mask_src = 1 - ((src == 1)*1)
            mask_src = mask_src.type(torch.bool).to(device)
            mask_tgt = 1 - ((tgt == 1)*1)
            mask_tgt = mask_tgt.type(torch.bool).to(device)
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        # dataset.keys() = dict_keys(['src', 'tgt', 'src_sent_labels', 'segs', 'clss', 'src_txt', 'tgt_txt'])
        src = ex["src"]
        tgt = ex["tgt"][: self.args.max_tgt_len][:-1] + [8002]  # 8002 : [EOS]
        ...
        end_id = [src[-1]]   # end_id is 3,[SEP] (b.c make sentence structure [CLS]+tokens+[SEP]
        src = src[:-1][: self.args.max_pos - 1] + end_id
        segs = segs[: self.args.max_pos]
        max_sent_id = bisect.bisect_left(clss, self.args.max_pos)
        src_sent_labels = src_sent_labels[:max_sent_id]
        clss = clss[:max_sent_id]
        # src_txt = src_txt[:max_sent_id]SS
