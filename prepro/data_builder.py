...
class BertData:
    def __init__(self, args, vocab, tokenizer):
        ...
        self.cls_idx = self.vocab["[CLS]"]     # 2
        self.sep_idx = self.vocab["[SEP]"]     # 3
        self.mask_idx = self.vocab["[MASK]"]   # 4
        self.bos_idx = self.vocab["[BOS]"]     # 8003
        self.eos_idx = self.vocab["[EOS]"]     # 8002
...
def format_to_bert(args):
    if (args.format_mode == 'raw'):
        if (args.dataset != ''):
            datasets = [args.dataset]
        else:
            datasets = ['Training/', 'Validation/'] # 'Training/' , 'Inference'
        for corpus_type in datasets:
            path = pjoin(args.raw_path, corpus_type+'magazine.jsonl')
            ...
            os.makedirs(args.json_path+corpus_type+args.data_type, exist_ok=True)
            os.makedirs(args.save_path, exist_ok=True)
            for i in range(math.ceil(len(data) / 2000)):
                tmp_path = args.json_path+ corpus_type + os.path.splitext(path.split('/')[-1])[0] + f'/{i}.jsonl'
            ...
            a_lst = []
            # for json_f in glob.glob(pjoin(args.json_path, '*' + args.corpus_type + '.*.jsonl')):
            #     real_name = json_f.split('/')[-1]
            for jsonl_f in sorted(glob.glob(args.json_path+corpus_type+args.data_type+"/*.jsonl")):
                real_name = jsonl_f.split('/')[-1]
                a_lst.append((args.corpus_type, jsonl_f, args, vocab, tokenizer, pjoin(args.save_path, real_name.replace('jsonl', 'bert.pt'))))
    ...
    try:
        for d in jobs:
            ...
            for s in d['text']:
                nd += s
            d['text'] = nd
            source, tgt =[tokenizer(s['sentence']) for s in d['text']], tokenizer(d['abstractive'][0])

            if args.use_anno_labels:
                if d['extractive'][0] != '':
                    sent_labels = d['extractive']
                    sent_labels = list(filter(None, sent_labels))
            else:
